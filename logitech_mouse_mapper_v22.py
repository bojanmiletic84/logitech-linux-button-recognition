#!/usr/bin/env python3
"""
Logitech LIFT Button Mapper – v23
=================================
- Wider mouse schematic (as in original)
- Modern gesture syntax (type "Gestures")
...
# ---------------------------------------------------------------------------
def _action_body_for_keys(*keys):
    return '{ type: "Keypress"; keys: [' + ", ".join(f'"{k}"' for k in keys) + ']; }'

def _action_body_for_special(action_type, **kwargs):
    if action_type == "ToggleSmartShift":
        return '{ type: "ToggleSmartShift"; }'
    elif action_type == "ToggleHiresScroll":
        return '{ type: "ToggleHiresScroll"; }'
    elif action_type == "CycleDPI":
        return '{ type: "CycleDPI"; sensors: [0]; }'
    else:
        return None

KNOWN_FUNCTIONS = {
    "— Default (leave unchanged) —": "DEFAULT",
    "Copy  (Ctrl+C)":            lambda: _action_body_for_keys("KEY_LEFTCTRL", "KEY_C"),
    "Paste  (Ctrl+V)":           lambda: _action_body_for_keys("KEY_LEFTCTRL", "KEY_V"),
    "Cut  (Ctrl+X)":             lambda: _action_body_for_keys("KEY_LEFTCTRL", "KEY_X"),
    "Undo  (Ctrl+Z)":            lambda: _action_body_for_keys("KEY_LEFTCTRL", "KEY_Z"),
    "Redo  (Ctrl+Shift+Z)":      lambda: _action_body_for_keys("KEY_LEFTCTRL", "KEY_LEFTSHIFT", "KEY_Z"),
    "Select All  (Ctrl+A)":      lambda: _action_body_for_keys("KEY_LEFTCTRL", "KEY_A"),
    "Save  (Ctrl+S)":            lambda: _action_body_for_keys("KEY_LEFTCTRL", "KEY_S"),
    "Find  (Ctrl+F)":            lambda: _action_body_for_keys("KEY_LEFTCTRL", "KEY_F"),
    "Browser Back  (Alt+Left)":  lambda: _action_body_for_keys("KEY_LEFTALT", "KEY_LEFT"),
    "Browser Forward (Alt+Right)": lambda: _action_body_for_keys("KEY_LEFTALT", "KEY_RIGHT"),
    "Back key  (XF86Back)":       lambda: _action_body_for_keys("KEY_BACK"),
    "Forward key  (XF86Forward)": lambda: _action_body_for_keys("KEY_FORWARD"),
    "Switch Window  (Alt+Tab)":  lambda: _action_body_for_keys("KEY_LEFTALT", "KEY_TAB"),
    "Show Desktop  (Super+D)":   lambda: _action_body_for_keys("KEY_LEFTMETA", "KEY_D"),
    "Close Window  (Alt+F4)":    lambda: _action_body_for_keys("KEY_LEFTALT", "KEY_F4"),
    "New Tab  (Ctrl+T)":         lambda: _action_body_for_keys("KEY_LEFTCTRL", "KEY_T"),
    "Close Tab  (Ctrl+W)":       lambda: _action_body_for_keys("KEY_LEFTCTRL", "KEY_W"),
    "Zoom In  (Ctrl++)":         lambda: _action_body_for_keys("KEY_LEFTCTRL", "KEY_EQUAL"),
    "Zoom Out  (Ctrl+-)":        lambda: _action_body_for_keys("KEY_LEFTCTRL", "KEY_MINUS"),
    "Play / Pause":              lambda: _action_body_for_keys("KEY_PLAYPAUSE"),
    "Next Track":                lambda: _action_body_for_keys("KEY_NEXTSONG"),
    "Previous Track":            lambda: _action_body_for_keys("KEY_PREVIOUSSONG"),
    "Volume Up":                 lambda: _action_body_for_keys("KEY_VOLUMEUP"),
    "Volume Down":               lambda: _action_body_for_keys("KEY_VOLUMEDOWN"),
    "Mute":                      lambda: _action_body_for_keys("KEY_MUTE"),
    "Screenshot  (PrintScreen)": lambda: _action_body_for_keys("KEY_PRINT"),
    "Middle Click  (button 3)":  lambda: _action_body_for_keys("BTN_MIDDLE"),
    "Toggle SmartShift":         lambda: _action_body_for_special("ToggleSmartShift"),
    "Toggle Hi-Res Scroll":      lambda: _action_body_for_special("ToggleHiresScroll"),
    "Cycle DPI":                 lambda: _action_body_for_special("CycleDPI"),
...
class GestureHelpDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("How to use gestures")
        self.setMinimumWidth(500)
        lay = QVBoxLayout(self)
        text = QLabel(
            "<h2>Using gestures on your Logitech LIFT</h2>"
            "<p><b>1. Configure</b> – assign a click action and/or gestures for up/down/left/right.</p>"
            "<p><b>2. Press & hold</b> the button (e.g., thumb button or wheel click).</p>"
            "<p><b>3. Move the whole mouse</b> in the desired direction while holding the button.</p>"
            "<p><b>4. Release</b> – the action triggers on release.</p>"
            "<p>If you assign a click action, it triggers when you press & release without moving.</p>"
            "<hr>"
            "<p><b>Important:</b> Not all buttons support gestures. On the LIFT, the "
            "<b>thumb buttons (forward/back) and the wheel click</b> are gesture‑capable.</p>"
            "<p>If gestures don't work on the wheel, try the <b>forward thumb button</b> first.</p>"
            "<p>For debugging, run <code>sudo logid -v</code> and watch for <code>Gesture: ...</code> messages.</p>"
        )
...
# ---------------------------------------------------------------------------
# Schematic widget - wider mouse shape
# ---------------------------------------------------------------------------
class MouseSchematic(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.setMinimumWidth(400)   # ensure it stays wide
    ...
    def _body_rect(self):
        ...
        m = 0.08   # reduced from 0.12 to make the mouse wider
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._gesture_supported_cids = set()
        ...
        left.addWidget(hint)
        root.addLayout(left, 4)   # give more space to the mouse
        ...
        self.natural_scroll_check.setToolTip("Reverse scroll direction (syncs with GNOME)")
        ...
        self.help_btn = QPushButton("Gesture Help")
        self.help_btn.clicked.connect(self._show_gesture_help)
        ...
        root.addLayout(right, 3)  # right panel slightly less
    ...
    def _rebuild_map(self):
        ...
        for i, (key, label, cid, _spec) in enumerate(active, start=1):
            ...
            cid_lower = cid.lower()
            if cid_lower not in self._gesture_supported_cids:
                gesture_btn.setEnabled(False)
                gesture_btn.setToolTip("This button does not support gestures")
            else:
                gesture_btn.setToolTip("Assign actions to up/down/left/right movements")
    ...
    def _edit_gesture(self, key):
        cid = self.rows[key][0].text().strip().lower()
        if cid not in self._gesture_supported_cids:
            QMessageBox.information(self, "Gesture Not Supported",
                                    f"Button '{key}' (CID {cid}) does not support gestures.\n"
                                    "Only certain buttons like thumb buttons and the wheel can be used for gestures.")
            return
    ...
    # -----------------------------------------------------------------------
    # Config parser (updated for new Gestures format)
    # -----------------------------------------------------------------------
    def _load_current_config(self):
        ...
        # Parse evdev / scroll / DPI / smartshift
        evdev_match = re.search(r'evdev\s*:\s*\(\s*\{[^}]*type\s*:\s*"REL_WHEEL"[^}]*direction\s*:\s*"Reversed"', chosen_block, re.DOTALL)
        if evdev_match:
        ...
        dpi_match = re.search(r'dpi\s*:\s*(\d+)', chosen_block, re.DOTALL)
        if dpi_match:
        ...
        for block in button_blocks:
            ...
            if 'type: "Gestures"' in action_body:
                gestures_match = re.search(r'gestures\s*:\s*\(\s*((?:[^()]|\([^()]*\))*)\)', action_body, re.DOTALL)
                if gestures_match:
                    gestures_str = gestures_match.group(1)
                    for gesture_entry in re.finditer(r'\{([^}]*)\}', gestures_str, re.DOTALL):
                        entry = gesture_entry.group(1)
                        direction_match = re.search(r'direction\s*:\s*"([^"]+)"', entry)
                        if not direction_match:
                            continue
                        direction = direction_match.group(1).lower()
                        inner_action_match = re.search(r'action\s*=\s*\{([^}]*)\}', entry, re.DOTALL)
                        if not inner_action_match:
                            continue
                        inner_body = inner_action_match.group(1)
                        if direction == "none":
                            func_name = self._parse_action_to_function(inner_body)
                            if func_name:
                                self._set_button_mapping(button_key, func_name, None)
                        else:
                            func_name = self._parse_action_to_function(inner_body)
                            if func_name:
                                if button_key not in self._gesture_actions:
                                    self._gesture_actions[button_key] = {}
                                self._gesture_actions[button_key][direction] = func_name
            else:
                func_name = self._parse_action_to_function(action_body)
                if func_name:
                    self._set_button_mapping(button_key, func_name, None)
    ...
    def _parse_action_to_function(self, action_body):
        for special_type, name in SPECIAL_ACTIONS.items():
            if f'type: "{special_type}"' in action_body:
                return name
        if 'type: "Keypress"' in action_body:
    ...
    # -----------------------------------------------------------------------
    # GNOME integration – sync natural scroll setting
    # -----------------------------------------------------------------------
    def _set_gnome_natural_scroll(self, enabled):
        desktop = os.environ.get("XDG_CURRENT_DESKTOP", "").lower()
        if "gnome" not in desktop and "ubuntu" not in desktop:
            return
        if not shutil.which("gsettings"):
            self._log("gsettings not found; cannot sync GNOME setting.")
            return
        value = "true" if enabled else "false"
        try:
            subprocess.run(
                ["gsettings", "set", "org.gnome.desktop.peripherals.mouse", "natural-scroll", value],
                check=True,
                timeout=5
            )
            self._log(f"GNOME natural scrolling set to {value}")
        except subprocess.CalledProcessError as e:
            self._log(f"Failed to set GNOME natural scrolling: {e}")
        except Exception as e:
            self._log(f"Error setting GNOME natural scrolling: {e}")

    # -----------------------------------------------------------------------
    # Build config – using modern Gestures format
    # -----------------------------------------------------------------------
    def build_config(self):
        ...
        if self.natural_scroll_check.isChecked():
            ...
            lines.append("    (")
            lines.append("      {")
            lines.append('        type: "REL_WHEEL";')
            lines.append('        direction: "Reversed";')
            lines.append("      }")
            lines.append("    );")
        ...
        def action_body_for_func(func_name):
            if func_name == "CUSTOM" or func_name == "DEFAULT" or func_name.startswith("Custom"):
                return None
        ...
        for key, (cid_edit, combo, gesture_btn) in self.rows.items():
            ...
            click_body = None
            if name != "DEFAULT" and name != "CUSTOM":
                click_body = action_body_for_func(name)
            elif name == "CUSTOM":
                keys = self._custom_keys.get(key)
                if keys:
                    click_body = _action_body_for_keys(*keys)

            gesture_directions = self._gesture_actions.get(key, {})
            gesture_entries = []

            if click_body:
                gesture_entries.append(f'{{ direction: "None"; mode: "OnRelease"; action = {click_body}; }}')

            for direction, func_name in gesture_directions.items():
                ...
                if func_name == "Custom keystroke…":
                    ...
                    if custom_keys:
                        action_body = _action_body_for_keys(*custom_keys)
                        gesture_entries.append(
                            f'{{ direction: "{direction_cap}"; mode: "OnRelease"; action = {action_body}; }}'
                        )
                else:
                    body = action_body_for_func(func_name)
                    if body:
                        gesture_entries.append(
                            f'{{ direction: "{direction_cap}"; mode: "OnRelease"; action = {body}; }}'
                        )

            if gesture_entries:
                block = f"      {{\n        cid: {cid};"
                block += "\n        action = {\n          type: \"Gestures\";\n          gestures: (\n            " + ",\n            ".join(gesture_entries) + "\n          );\n        };"
                block += "\n      }"
                blocks.append(block)
            else:
                if click_body:
    ...
    def preview_config(self):
        ...
        self._log(cfg)
        if self._gesture_actions:
            self._log("\n--- Configured gestures ---")
            for key, gestures in self._gesture_actions.items():
                self._log(f"{key}: {gestures}")
    ...
    def _detection_done(self, out):
        ...
        # Parse gesture support
        self._gesture_supported_cids.clear()
        for line in out.splitlines():
            if "gesture support?" in line:
                continue
            m = re.search(r"0x([0-9a-fA-F]+)\s*\|.*\|.*\|.*\|.*\bYES\b", line)
            if m:
                cid = "0x" + m.group(1).lower()
                self._gesture_supported_cids.add(cid)
                self._log(f"Gesture support detected for CID {cid}")
        if not self._gesture_supported_cids:
            self._log("No gesture support detected from logid; falling back to known LIFT gesture CIDs")
            self._gesture_supported_cids.update(["0x52", "0x53", "0x56", "0xd7", "0xfd"])
    ...
    def save_and_apply(self):
        ...
        self._set_gnome_natural_scroll(self.natural_scroll_check.isChecked())
        self._check_service_status()
